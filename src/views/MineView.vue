<template>
    <div class="h-screen w-screen bg-[#F5F5F5]">
        ...
        <Icon icon="circum:search" color="#78758b" class="w-[4vw] h-[8vw] absolute left-[11vw] " @click.native="searchHandler(userSearchKeywords)"/>
        ...
        <div class="w-[90vw] h-[21vw] mx-auto mt-[3vw]">
            <b class="text-[4.5vw]">猜你喜欢</b>
            <div class="flex mt-[3vw] text-xs justify-around text-center leading-[10vw] text-[#8B9091]">
                <div class="w-[24vw] h-[9vw] bg-white rounded-2xl"> Hard To Get</div>
                <div class="w-[14vw] h-[9vw] bg-white rounded-2xl">蔡徐坤</div>
                <div class="w-[11vw] h-[9vw] bg-white rounded-2xl">雨爱</div>
                <div class="w-[20vw] h-[9vw] bg-white rounded-2xl">夏天的风</div>
            </div>
                ...
                <van-swipe :loop="false" :width="300" :show-indicators="false">
                    <van-swipe-item v-for="(item,index) in rankDate" :key="item.id" class="w-[64.3vw]">
                        <div class="w-[64.3vw] mr-[2.87vw] px-[4vw] bg-white dark:bg-[#] rounded-xl">
                            ...
                            <ul class="w-[64.3vw] ">
                                <li class="h-[13.56vw] leading-[13.56vw] font-[800] flex items-center" v-for="(item2,index2) in item.tracks.slice(0,20)" :key="item2">
...
export default {
    ...
    async created() {
        ...
        const res = await axios.get('https://netease-cloud-music-api-five-roan-88.vercel.app/toplist/detail');
                ...
                this.rankDate=playlist.map(item => item.data.playlist).slice(0,9)
                console.log(this.rankDate);
